...
export class ElGamal {
    ...
    public static getElGamal(): ElGamal{
        if(!this.elGamal)
    ...
    public verifyDigitalSignature(message: string, digitalSignature: BigInteger[], p: BigInteger, g: BigInteger, y: BigInteger): boolean {
        const hash = crypto.createHash('sha256').update(message, 'utf8').digest();
