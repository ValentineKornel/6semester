...
class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        using (IRepository repository = Repository.Create("..\\..\\..\\Celebrities"))
        {
            ...
            if (testdel1 != null)
                if (repository.deleteSelebrity((int)testdel1)) Console.WriteLine($" delete {testdel1}"); else Console.WriteLine($" delete {testdel1} error");
            if (testdel2 != null)
                if (repository.deleteSelebrity((int)testdel2)) Console.WriteLine($" delete {testdel2}"); else Console.WriteLine($" delete {testdel2} error");
            if (repository.deleteSelebrity(1000)) Console.WriteLine($" delete {1000}"); else Console.WriteLine($" delete {1000} error");
            repository.SaveChanges();
            print("del 2");

            if (testupd2 != null)
            {
                if (repository.updateSelebrity((int)testupd1, new Celebrity((int)testupd1, "Edgar_upd", "Codd_upd", "D:\\Study\\university\\TPWI\\lab4\\ASPA\\Test_DAL003\\Celebrities\\Codd.jpg")) != null)
